...
## 파이썬의 속도를 개선하는 방법
* 같은 일을 수행하는 코드이더라도, 파이썬에서는 함수로 감싸서 사용하는게 훨씬 빠르다.
    * 일반적으로는 함수를 이용하면 오버헤드, 시스템 스택 할당 등 느리다고 생각되지만, 
    * 파이썬에서는 global 변수와 local 변수에 속도차이로 함수로 감싸는 것이 더 빠르게 작동한다.
* 입력 시, sys.stdin.readline()을 이용. 배열 원소 출력 시, "".join(arr) 이용
...
* 재귀적인 함수를 작성하다 보면, 큰 수의 계산이 반복되는 문제가 있다. 
* 최종적으로 구해야 되는 결과가 어떤 수로 나눈 나머지라면, 모든 계산 과정에서 모듈로 연산을 하며 값을 저장(memoization)하는 것이 훨씬 시간 효율적이다.
* 일반적인 시각으로, 모든 연산과정에 하나씩 연산이 추가되니 더 늦어질거라 생각이 되지만, 
* 마지막에 나눈것만이 아닌 모든 dp를 구하는 과정에서 값을 더하거나 옮기는 것들도 모두 이 자릿수에 비례하게 시간이 걸리기 때문에 오히려 시간 효율적이다.
...
* ':=" 바다코끼리 연산자
    ...
    * 즉, 변수 = 표현식을 하고, return 변수 기능을 합니다.
    * 파이썬 3.8부터 추가
...
* zip()
    * zip() 함수는 여러 개의 순회 가능한(iterable) 객체를 인자로 받고, 각 객체가 담고 있는 원소를 튜플의 형태로 차례로 접근할 수 있는 반복자(iterator)를 반환한다.
        ...
        (3, 'C')
        ```
    * zip()과 unpack('*')을 사용하면, numpy를 이용하지 않고도, 전치행렬을 간단하게 만들 수 있다.
    * <img src=../rsc/img/python/zip_unpack.png>

* zip_longest(fillvalue=)
    * zip()은 리스트의 길이가 상이할 경우 가장 짧은 리스트의 길이만큼만 적용된다.
    * 긴 리스트를 대상으로 zip()을 사용하고 싶을 때에는 itertools.zip_longest()를 사용하면 된다. 
    * itertools 라이브러리의 zip_longest(*iterables, fillvalue=None)는 fillvalue= 인자에 값을 지정하여 zip() 함수를 적용해준다.
...
* **from heapq import heappush, heappop** ... 등 다양한 힙관련 함수들이 제공된다.
    * heapq 모듈은 이진트리 기반의 최소 힙 자료구조를 제공한다.
    * 이는 자바의 PriorityQueue 클래스와 비슷하다.
    * 최소 힙(min heap)을 사용하면 원소들이 항상 정렬된 상태로 추가되고 삭제된다.
    * 또한, min heap에서 가장 작은 값은 인덱스 0, 즉 이진트리의 루트에 위치한다.
    * 파이썬에서 최소 힙은 일반적인 리스트를 heapq 모듈을 통해 다루는 것을 말한다.
    * 내부적으로 이진트리를 다루는 heappush()와 heappop()은 O(log(n))의 시간복잡도를 갖는다.
    * 인덱스 0은 항상 최소값이 위치하지만, **인덱스 1에 두번쨰 작은 원소, 인덱스 2에 세번째 작은 원소가 있다는 보장은 없다.** 두번째 작은 원소를 얻기위해선 반드시 heappop()을 수행 후 heap[0]을 통해 새로운 최소값에 접근한다.
    * heapify() 함수는 기존 리스트를 힙으로 변환한다. O(n)의 시간복잡도를 갖는다.
    * nsmallest() : 주어진 리스트에서 가장 작은 n개의 값을 담은 리스트를 반환한다. 그 결과 리스트의 마지막 값이 n번째 작은 값이 된다. <=> nlargest() 함수도 존재한다.
* **from queue import PriorityQueue** 
    * 우선순위 큐를 제공한다.
    * heapq를 기반으로 만들어졌다. **쓰레드 안정적이지만, heapq에 비해 속도가 느리다.**
        ```python
        pq = PriorityQueue()
        pq.put(3)
        pq.get()
