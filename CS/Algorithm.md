...
* 하지만 가중 그래프에 최단 경로를 구하기 위해선 더 정교한 알고리즘이 필요하다.
    * 벨만-포드 알고리즘(Bellman-Ford algorithm)
        ...
        * 다른 방법으론 SPFA(shotest path fater algorithm), 거리를 줄이는데 사용될 수 있는 노드를 별도의 큐로 관리, 그 노드만 처리한다.
...
* 후속 노드 그래프(successor grpah, functional graph)
    ...
    * 크루스칼 알고리즘(kruskal's algorithm)
        ...
        * 유니온-파인드 자료구조(union-find structure)
            ...
            * 구현
            ...
            ```
            * 경로의 길이가 O(log n)이라 하면, find 함수의 시간복잡도도 O(log n)이 된다.
            * 이 경우 same과 unite도 O(log n) 시간에 실행된다.

            * 경로 압축(path compression)
                * find 함수를 변형시켜 집합의 모든 원소가 대푯값을 바로 가리키게 할 수 있다.
                ```
                int find(int x) {
                ...
                ```
                * 이를 통해 유니온-파인드 연산의 분할 상환에 따른 시간 복잡도가 O(a(n)) (a는 알파, 애커만 함수의 역함수). 이는 상수와 비슷하다.
    
    * 프림 알고리즘(Prim's algorithm)
        * MST를 구하는 또 다른 방법, 임의의 노드를 트리에 추가하고, 새로운 노드와 연결하는 노드 중 가중치가 가장 작은 간선을 선택해간다.
        * 다익스트라와 유사. 차이점은 시작지점이 아닌, '트리에서' 가중치가 최소인 간선으로 연결되는 노드를 구한다.
        * 우선순위 큐를 이용하여 현재 컴포넌트와 간선 하나로 연결된 모든 노드를 저장하고, 연결에 사용될 간선 가중치의 오름차순으로 정렬해 하나씩 뽑아간다.
        * 프림 알고리즘은 O(n + m log m)시간에 동작한다.
